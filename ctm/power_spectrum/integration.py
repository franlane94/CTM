...
"""
def PowerIntegral2(self, ki, q, n_max, front, exponent_1, zero_lag, P, A, kmax):

    P_sum = 0.0

    if ki <= 5e-4:

        P_sum = calc_low_k_approx(ki, P, zero_lag, A, kmax)
        ...
        for n in range(0, n_max+1):
            ...
            for p in range(n+1):
                ...
                P_sum += p_func(ki, q, n, p, front, exponent_1, zero_lag)

def p_func(ki, q, n, p, front, exponent_1, zero_lag):

    I = ZeldovichPowerIntegral2(q, 2*p)

    if n == 0:

        sum_p = np.exp(-0.5*ki**2*exponent_1)-np.exp(-ki**2*zero_lag)

    else:

        sum_p = np.exp(-0.5*ki**2*exponent_1)*(-0.5*ki**2*front)**n*(factorial(2*n)/(np.power(2, n)*factorial(n)))*np.power(-2, p)*((4.0*p+1.0)/(factorial(n-p)*factorial2(2*n+2*p+1)))

    kk, this_Pzel = I(sum_p, extrap=False)

    P = spline(kk, this_Pzel)(ki)

    return P

    class ZeldovichPowerIntegral2(mcfit.mcfit):
